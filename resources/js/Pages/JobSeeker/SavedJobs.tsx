...
import ImageInitialsFallback from "@/Components/ImageInitialsFallback"; // Add this import
/* ── Types ── */
...
/* ── Updated statusLabel to match BrowseJobs logic ── */
function statusLabel(stage?: string | null) {
    const s = (stage || "").toString().toLowerCase();
    if (s === "interviewing" || s === "approved" || s === "hired")
        return "Hired";
...
/* ─────────────────────────────────────────
   SAVED JOB CARD (Matched to BrowseJobs)
───────────────────────────────────────── */
function SavedJobCard({
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-md hover:border-gray-300 transition-all flex flex-col gap-4 min-w-0 overflow-hidden">
            {/* Avatar + Title (Uses dynamic colors & initials) */}
            <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1 pt-0.5">
                    <h3 className="text-[15px] font-bold text-avaa-dark leading-snug">
                        {job.title}
                    </h3>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pt-4 border-t border-gray-100 mt-auto">
                <div className="min-w-0 flex-shrink-0">
                    ...
                    <p className="text-base font-extrabold text-avaa-dark">
                        {salary || "Negotiable"}
                    </p>
                </div>

                <div className="flex flex-wrap items-center gap-2 min-w-0 flex-1 justify-end">
                    {/* 2. Update Bookmark Button to handle onClick */}
                    <button
                        onClick={onUnsave}
                        title="Remove from saved"
                        className="p-2.5 rounded-xl border border-avaa-primary/30 bg-avaa-primary-light text-avaa-teal hover:bg-avaa-primary hover:text-white transition-all flex-shrink-0"
                    >
                        <svg
                            width="16"
                            height="16"
                    ...
                    <button
                        onClick={onView}
                        className="px-4 py-2 border border-gray-300 text-gray-700 hover:border-avaa-primary hover:text-avaa-teal text-sm font-semibold rounded-xl transition-colors"
                    >
                        View Details
                    </button>
                    ...
                    <button
                        onClick={onApply}
                        disabled={isDisabled}
                        className={`px-4 py-2 text-sm font-semibold rounded-xl transition-colors disabled:opacity-80 disabled:cursor-not-allowed min-w-0 ${
                            isRejected || isEnded
                    ...
                    >
                        <span className="inline-flex items-center gap-1.5">
                            {applyLabel}
                            {/* Target icon for Hired/Interviewing status per photo 1 */}
                            {(isHired ||
                                status === "interviewing" ||
                                status === "approved") && (
                                <span
                            ...
                            {isApplied && !isHired && status === "pending" && (
                                <span
                                    aria-hidden="true"
                                    className="text-[12px] leading-none"
                                >
...
function DrawerFilterSection({
    ...
    return (
        <div>
            <p className="text-xs font-bold text-avaa-dark uppercase tracking-wider mb-3">
                {title}
            </p>
            {children}
        </div>
...
export default function SavedJobs({
    ...
    // 3. Add this function to handle the Unsave action
    const handleToggleSave = (jobId: number) => {
        // Use delete since we are explicitly 'unsaving' here
        router.delete(route("job-seeker.jobs.unsave", jobId), {
            preserveScroll: true,
            onSuccess: () => {
                // This line handles the local UI hide
    ...
    const handleApply = (jobId: number) => {
        router.post(
            route("job-seeker.jobs.apply", jobId),
            {},
            {
                preserveScroll: true,
                onSuccess: () =>
                    setJobs((prev) =>
                        prev.map((j) =>
                            j.id === jobId ? { ...j, has_applied: true } : j,
                        ),
                    ),
            },
        );
    ...
    const handleView = (jobId: number) =>
        router.visit(
            route("job-seeker.jobs.show", { job: jobId, from: "saved" }),
        );
    ...
    return (
        <AppLayout activeNav="Saved Jobs" pageTitle="Saved Jobs">
            ...
            >
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-base font-bold text-avaa-dark">
                            Filters
                        </span>
                        {activeFilterCount > 0 && (
            ...
            <div className="mb-6 sm:mb-8">
                <h1 className="text-2xl sm:text-3xl font-extrabold text-avaa-dark">
                    Saved Jobs
                </h1>
                <p className="text-sm sm:text-base text-avaa-muted mt-2">
                    Browse the jobs you've saved.
                </p>
            </div>
            ...
            <div className="flex gap-8 min-w-0 overflow-x-hidden">
                <aside className="hidden lg:flex flex-col gap-7 w-72 flex-shrink-0">
                    ...
                    <div>
                        <p className="text-sm font-bold text-avaa-dark mb-3">
                            Date Posted
                        </p>
                        <div className="flex flex-wrap gap-2">
                            {DATE_FILTERS.map((f) => (
                ...
                <div className="flex-1 min-w-0">
                    {jobs.length === 0 ? (
                        <div className="bg-white rounded-2xl border border-gray-200 p-20 text-center">
                            ...
                            <p className="text-lg font-semibold text-avaa-dark mb-1">
                                No saved jobs yet
                            </p>
                            <p className="text-base text-avaa-muted mb-6">
                                Save jobs while browsing to find them here
                                later.
                            </p>
                            <button
                                onClick={() =>
