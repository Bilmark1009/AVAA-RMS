...
function JobDetailModal({ job, onClose, onApply }: {
    job: JobListing; onClose: () => void; onApply: () => void;
}) {
    useEffect(() => {
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {job.skills_required.map(s => (
                                        <span key={s} className="px-2.5 py-0.5 bg-avaa-primary-light text-avaa-teal text-xs font-semibold rounded-full border border-avaa-primary/20">{s}</span>
                                    ))}
                                </div>
                            </div>
                        )}
...
/* ── Saved Job Card ── */
function SavedJobCard({ job, onUnsave, onApply, onView }: {
    job: JobListing; onUnsave: () => void; onApply: () => void; onView: () => void;
}) {
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 hover:shadow-md hover:border-gray-300 transition-all flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-100 mt-auto">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={onView}
                        className="px-4 py-1.5 border border-gray-200 text-gray-600 hover:border-avaa-primary/40 hover:text-avaa-teal text-xs font-semibold rounded-xl transition-colors whitespace-nowrap">
                        View Details
                    </button>
                    <button onClick={onApply} disabled={job.has_applied}
...
export default function SavedJobs({ user, savedJobs, filters, availableSkills, availableCompanies }: Props) {
    ...
    // Keep local list in sync when Inertia re-renders (after unsave)
    ...
    const handleApply = (jobId: number) => {
        router.post(route('job-seeker.jobs.apply', jobId), {}, {
            ...
            onSuccess: () => {
                ...
                setViewJob(v => v?.id === jobId ? { ...v, has_applied: true } : v);
            },
        });
    };
    ...
    return (
        <AppLayout
            ...
            pageSubtitle="Browse the jobs you've saved."
            activeNav="Saved">
            ...
            {/* Page heading */}
            <div className="mb-5">
                <h1 className="text-xl font-extrabold text-avaa-dark">Saved Jobs</h1>
                <p className="text-sm text-avaa-muted mt-0.5">Browse the jobs you've saved.</p>
            </div>
                ...
                <div className="flex-1 min-w-0">
                    {jobs.length === 0 ? (
                        <div className="bg-white rounded-2xl border border-gray-200 p-16 text-center">
                            ...
                            <p className="font-semibold text-avaa-dark mb-1">No saved jobs yet</p>
                            <p className="text-sm text-avaa-muted mb-5">Save jobs while browsing to find them here later.</p>
                            <button onClick={() => router.visit(route('job-seeker.jobs.browse'))}
                                className="px-5 py-2.5 bg-avaa-primary hover:bg-avaa-primary-hover text-white text-sm font-semibold rounded-xl transition-colors">
                                Browse Jobs
                            </button>
                        </div>
                    ) : (
